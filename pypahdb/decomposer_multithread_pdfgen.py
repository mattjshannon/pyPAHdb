...
def _plot_fit(input_tuple):
    ...
    """
    i, j, abscissa, charge, data, model, norm, size, \
        large_fraction, ionized_fraction = input_tuple
    ...
    ax2.plot(abscissa, size['large'][:, i, j],
             label='large', lw=1, color='purple')
    ax2.plot(abscissa, size['small'][:, i, j],
             label='small', lw=1, color='crimson')
    size_str = "$f_{large}$=%3.1f" % (large_fraction[i][j])
    ...
    ion_str = "$f_{ionized}$=%3.1f" % (ionized_fraction[i][j])
    ...
    tmp_name = 'plot_' + str(i) + '_' + str(j) + '.pdf'
    fig.savefig('plots/' + tmp_name)
    plt.close()
    fig.clear()
...
class Decomposer(DecomposerBase):
    ...
    def save_pdf(self, filename, header="", domaps=True, doplots=True):
        ...
        if(doplots):
            ...
            abscissa = self.spectrum.spectral_axis
            charge = self.charge
            data = self.spectrum.flux.T
            model = self.fit
            norm = self.norm
            size = self.size
            large_fraction = self.large_fraction
            ionized_fraction = self.ionized_fraction

            input_tuples = ((i, j, abscissa, charge, data, model, norm, size, large_fraction, ionized_fraction) for i in range(ordinate.shape[1]) for j in range(ordinate.shape[2]))
            max_workers = 4
            pool = ProcessPoolExecutor(max_workers=max_workers)
            results = list(pool.map(_plot_fit, input_tuples))
